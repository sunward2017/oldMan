...
class DrugInfo extends Component{
  ...
  getUnitList(){
    const {customerId} = this.props.auth;
    httpServer.listParam({customerId,type:1},{headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then((res)=>{
      if (res.code === 200) {
  ...
  render(){
    ...
    return(
      <div>
        ...
        {
          modalFlag?<ModalInfo cancleClick={this.handleCancel} unitList={unitList} record={record} flag={flag} action={action} getDrugList={()=>{this.getDrugList()}} customerId={this.props.auth.customerId}/> :null
        }
      </div>
    )
  }
