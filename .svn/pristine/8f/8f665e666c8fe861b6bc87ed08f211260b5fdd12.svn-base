...
axios.interceptors.request.use(function (config) {
    ...
    config.data = level===2 ? qs.stringify({...config.data, uId:id, client:'admin', cId: customerId, session,customerId}):qs.stringify({...config.data, uId:id, client:'client', cId: customerId, customerId,session});
    return config;
}, function (error) {
